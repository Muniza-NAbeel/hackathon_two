...
export default function EnhancedTasksPage() {
  ...
  const handleCreateTask = async (data: TaskCreate) => {
    ...
    try {
      ...
      showSuccess('Task Created', `"${created.title}" has been added to your list`)
      if (data.remind_at) {
        showInfo('Reminder Set', 'You will be notified before the due date')
      }
    } catch (err) {
      showError('Failed to Create', err instanceof Error ? err.message : 'Could not create task')
      throw err
    }
  }
  ...
  const handleUpdateTask = async (data: TaskUpdate) => {
    ...
    try {
      ...
      showError('Failed to Update', err instanceof Error ? err.message : 'Could not update task')
      throw err
    }
  }
  ...
  const handleDeleteTask = async (id: number) => {
    ...
    try {
      ...
      showSuccess('Task Deleted', taskToDelete ? `"${taskToDelete.title}" has been removed` : 'Task deleted')
    } catch (err) {
      showError('Failed to Delete', err instanceof Error ? err.message : 'Could not delete task')
    }
  }
  ...
  const handleToggleComplete = async (id: number) => {
    ...
    try {
      ...
      if (updated.status === 'completed') {
        if (taskBefore?.recurrence && taskBefore.recurrence !== 'none') {
          showSuccess('Recurring Task Completed', `"${updated.title}" completed! Next ${taskBefore.recurrence} task will be created automatically.`)
        } else {
          showSuccess('Task Completed', `"${updated.title}" marked as complete`)
        }
      } else {
        showInfo('Task Reopened', `"${updated.title}" marked as pending`)
      }
    } catch (err) {
      console.error('Failed to toggle task:', err)
      showError('Failed to Update', err instanceof ApiError ? err.message : 'Could not update task status')
    } finally {
