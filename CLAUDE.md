...
If specialized agents or skills are available for a task, Claude MUST use them.

Claude MUST NOT implement functionality directly if an agent or skill exists.

Only if no suitable agent or skill exists, Claude may proceed by:
1. Creating a temporary internal agent, or
2. Implementing the logic directly.

When an agent or skill is used, Claude MUST explicitly state:
- Which agent was used
- Which skill was executed

For voice-based input or natural language command parsing,
Claude MUST use the `voice-command-agent` if available.
